...
async def _enable_totp(client: AsyncClient, headers: dict) -> str:
    ...
    setup = await client.post("/api/users/me/totp/setup", headers=headers)
    assert setup.status_code == 200
    secret = setup.json()["secret"]

    code = pyotp.TOTP(secret).now()
    verify = await client.post("/api/users/me/totp/verify", headers=headers, json={"secret": secret, "code": code})
...
async def test_totp_setup_returns_secret_and_uri(client: AsyncClient, unique_username):
    ...
    resp = await client.post("/api/users/me/totp/setup", headers=headers)
...
async def test_totp_verify_enables_totp(client: AsyncClient, unique_username):
    ...
    resp = await client.post("/api/users/me/totp/verify", headers=headers, json={"secret": secret, "code": code})
...
async def test_totp_verify_wrong_code_rejected(client: AsyncClient, unique_username):
    ...
    resp = await client.post("/api/users/me/totp/verify", headers=headers, json={"secret": secret, "code": "000000"})
...
async def test_login_with_wrong_totp_code_rejected(client: AsyncClient, unique_username):
    ...
    resp = await client.post("/api/auth/token", data={"username": username, "password": password, "totp_code": "000000"})
    assert resp.status_code == 401
...
async def test_login_with_correct_totp_code_succeeds(client: AsyncClient, unique_username):
    ...
    code = pyotp.TOTP(secret).now()
    resp = await client.post("/api/auth/token", data={"username": username, "password": password, "totp_code": code})
    assert resp.status_code == 200
...
async def test_totp_disable_succeeds_with_valid_code(client: AsyncClient, unique_username):
    ...
    code = pyotp.TOTP(secret).now()
    resp = await client.request("DELETE", "/api/users/me/totp", headers=headers, json={"code": code})
    assert resp.status_code == 204
...
async def test_totp_disable_wrong_code_rejected(client: AsyncClient, unique_username):
    ...
    resp = await client.request("DELETE", "/api/users/me/totp", headers=headers, json={"code": "000000"})
    assert resp.status_code == 400
    assert resp.json()["detail"]["code"] == "INVALID_TOTP_CODE"
...
async def test_totp_disable_when_not_enabled_returns_400(client: AsyncClient, unique_username):
    ...
    resp = await client.request("DELETE", "/api/users/me/totp", headers=headers, json={"code": "123456"})
    assert resp.status_code == 400
