...
async def test_update_password(client: AsyncClient, unique_username):
    ...
    reg = await client.post("/auth/register", json={"username": username, "password": "oldpass123"})
    client.headers["Authorization"] = f"Bearer {reg.json()['access_token']}"

    resp = await client.patch("/users/me", json={"current_password": "oldpass123", "new_password": "newpass123"})
    assert resp.status_code == 200

    bad = await client.post("/auth/token", data={"username": username, "password": "oldpass123"})
    assert bad.status_code == 401

    good = await client.post("/auth/token", data={"username": username, "password": "newpass123"})
    assert good.status_code == 200


async def test_update_password_wrong_current(client: AsyncClient, unique_username):
    username = unique_username("pwwrong")
    reg = await client.post("/auth/register", json={"username": username, "password": "pass12345"})
    client.headers["Authorization"] = f"Bearer {reg.json()['access_token']}"

    resp = await client.patch("/users/me", json={"current_password": "wrongpass", "new_password": "newpass123"})
...
async def test_update_password_too_short(client: AsyncClient, unique_username):
    ...
    resp = await client.patch("/users/me", json={"current_password": "pass12345", "new_password": "short"})
...
async def test_admin_list_users_pagination(admin_client: AsyncClient):
    resp = await admin_client.get("/users?limit=1&offset=0")
    assert resp.status_code == 200
...
async def test_admin_can_get_user_by_id(admin_client: AsyncClient, client: AsyncClient, unique_username):
    ...
    resp = await admin_client.get(f"/users/{user_id}")
    assert resp.status_code == 200
    assert resp.json()["username"] == username


async def test_admin_get_nonexistent_user(admin_client: AsyncClient):
    resp = await admin_client.get(f"/users/{uuid.uuid4()}")
...
async def test_admin_can_promote_user_to_admin(admin_client: AsyncClient, client: AsyncClient, unique_username):
    ...
    resp = await admin_client.patch(f"/users/{user_id}/role", json={"global_role": "admin"})
    assert resp.status_code == 200
...
async def test_admin_can_demote_admin(admin_client: AsyncClient, client: AsyncClient, unique_username):
    ...
    await admin_client.patch(f"/users/{user_id}/role", json={"global_role": "admin"})
    resp = await admin_client.patch(f"/users/{user_id}/role", json={"global_role": "user"})
    assert resp.status_code == 200
...
async def test_admin_can_deactivate_user(admin_client: AsyncClient, client: AsyncClient, unique_username):
    ...
    login = await client.post("/auth/token", data={"username": username, "password": "securepass1"})
...
async def test_non_admin_cannot_update_role(client: AsyncClient, unique_username):
    ...
    resp = await client.patch(f"/users/{uuid.uuid4()}/role", json={"global_role": "admin"})
    assert resp.status_code == 403
