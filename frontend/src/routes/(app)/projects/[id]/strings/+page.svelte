						...
						<div class="border-b bg-muted/30 px-4 py-3">
							...
							{#if group.entries[0].comment}
								<p class="mt-1 break-words text-xs text-muted-foreground">
									...
									{#if group.entries[0].comment_auto_generated}
										<AutoGeneratedIcon />
									{/if}
								</p>
							{/if}
					...
					{#each grouped as group}
						{#if group.entries.length === 1 && group.entries[0].variation_type === 'none'}
							...
							<Table.Row class="hover:bg-muted/50">
								...
								<Table.Cell class="whitespace-normal break-words text-sm text-muted-foreground">
									{loc.comment ?? '—'}{#if loc.comment && loc.comment_auto_generated}<AutoGeneratedIcon />{/if}
								</Table.Cell>
								<Table.Cell>
									<span class="rounded-full px-2 py-0.5 text-xs font-medium {stateColors[loc.state]}">{loc.state.replace('_', ' ')}</span>
								</Table.Cell>
						...
						{:else}
							<Table.Row class="bg-muted/30 hover:bg-muted/50">
								...
								<Table.Cell class="whitespace-normal break-words text-sm text-muted-foreground">
									{group.entries[0].comment ?? '—'}{#if group.entries[0].comment && group.entries[0].comment_auto_generated}<AutoGeneratedIcon />{/if}
								</Table.Cell>
								<Table.Cell></Table.Cell>
							</Table.Row>
							{#each group.entries as loc}
				...
				<Table.Body>
					{#each strings.data?.items ?? [] as str}
						<Table.Row class={str.should_translate ? 'cursor-pointer hover:bg-muted/50' : 'opacity-50'}>
							...
							<Table.Cell class="whitespace-normal break-words text-sm text-muted-foreground">
								{str.comment ?? '—'}{#if str.comment && str.comment_auto_generated}<AutoGeneratedIcon />{/if}
							</Table.Cell>
